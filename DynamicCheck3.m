function [true_module, dia, face_width, F_end, F_dynamic, F_wear] = DynamicCheck3(face_width, true_module, dia, K, Q, strength, y, M_t, rpm, material1, material2, profile, N, speed)
...
while (F_dynamic > F_wear && F_dynamic > F_end) && (error1 >= 0.11 && error2 >= 0.11)
    ...
    if (F_dynamic < F_wear && F_dynamic < F_end) && (error1 < 0.11 && error2 < 0.11)
        continue
    elseif ((F_dynamic > F_wear && F_dynamic < F_end) || (F_dynamic < F_wear && F_dynamic > F_end)) && error1 < 0.11 && error2 < 0.11
